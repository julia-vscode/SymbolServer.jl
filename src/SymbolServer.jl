...
mutable struct SymbolServerProcess
    ...
    function SymbolServerProcess(;environment = nothing, depot = nothing)
        ...
        p = if environment === nothing
            ...
            open(pipeline(Cmd(`$jl_cmd --startup-file=no --history-file=no --project=$environment $server_script`, dir = environment, env = env_to_use), stderr = stderr_for_client_process), read = true, write = true)
        end
